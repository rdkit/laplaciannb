...
class TestLaplacianNBCompatibility:
    ...
    def test_same_predictions_binary_classification(self, generate_fingerprint_data):
        ...
        np.testing.assert_array_equal(pred_original, pred_new,
                                     err_msg="Binary predictions differ between implementations")
    ...
    def test_same_predictions_multiclass(self, generate_fingerprint_data):
        ...
        # Assert predictions are the same
        np.testing.assert_array_equal(pred_original, pred_new,
                                     err_msg="Multiclass predictions differ between implementations")
    ...
    def test_same_probabilities(self, generate_fingerprint_data):
        ...
        np.testing.assert_allclose(prob_original, prob_new, rtol=1e-5, atol=1e-8,
    ...
    def test_same_log_probabilities(self, generate_fingerprint_data):
        ...
        np.testing.assert_allclose(log_prob_original, log_prob_new, rtol=1e-4, atol=1e-7,
    ...
    def test_different_alpha_values(self, generate_fingerprint_data):
        ...
        for alpha in alpha_values:
            ...
            np.testing.assert_array_equal(pred_original, pred_new,
                                         err_msg=f"Predictions differ for alpha={alpha}")
    ...
    def test_sample_weights(self, generate_fingerprint_data):
        ...
        np.testing.assert_array_equal(pred_original, pred_new,
                                     err_msg="Predictions differ when using sample weights")
    ...
    def test_sparse_vs_dense_input(self, generate_fingerprint_data):
        ...
        np.testing.assert_array_equal(pred_sparse, pred_dense,
                                     err_msg="Predictions differ between sparse and dense input")
        
        # Compare probabilities
        prob_sparse = clf_sparse.predict_proba(X_sparse[:20])
        prob_dense = clf_dense.predict_proba(X_dense[:20])
        
        np.testing.assert_allclose(prob_sparse, prob_dense, rtol=1e-5, atol=1e-8,
    ...
    def test_feature_counting_consistency(self, generate_fingerprint_data):
        ...
        assert clf_original.feature_all_ == clf_new.feature_all_, \
            "Total feature counts differ between implementations"
        
        # Check class counts
        np.testing.assert_allclose(clf_original.class_count_, clf_new.class_count_,
                                  err_msg="Class counts differ between implementations")
    ...
    def test_single_class_edge_case(self):
        ...
        np.testing.assert_array_equal(pred_original, pred_new,
                                     err_msg="Single class predictions differ")
    ...
    def test_empty_features_handling(self):
        ...
        np.testing.assert_array_equal(pred_original, pred_new,
                                     err_msg="Predictions differ with empty fingerprints")
