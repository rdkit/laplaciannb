...
from laplaciannb.fingerprint_utils import convert_fingerprints, FingerprintTransformer
...
class TestSklearnIntegration:
    ...
    @pytest.mark.skipif(
        not pytest.importorskip("rdkit", reason="RDKit not available"),
        reason="RDKit required for this test"
    )
    ...
    def test_fingerprint_transformer(self, simple_fingerprint_data):
        ...
        pipeline = Pipeline([
            ('fingerprints', FingerprintTransformer(n_bits=50)),
            ('classifier', LaplacianNB_New())
        ])
    ...
    def test_transformer_pipeline_with_grid_search(self, simple_fingerprint_data):
        ...
        pipeline = Pipeline([
            ('fingerprints', FingerprintTransformer()),
            ('classifier', LaplacianNB_New())
        ])
