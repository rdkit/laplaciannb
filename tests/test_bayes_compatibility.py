...
class TestBayesCompatibility:
    ...
    def test_basic_bayes_scenario_compatibility(self):
        ...
        assert_array_equal(pred_original, pred_new, 
                          err_msg="Predictions don't match for basic bayes scenario")
        ...
        assert_allclose(clf_original.feature_count_, clf_new.feature_count_, rtol=1e-10,
                       err_msg="Feature counts don't match")
        assert_allclose(clf_original.class_count_, clf_new.class_count_, rtol=1e-10,
                       err_msg="Class counts don't match")
        assert_allclose(clf_original.feature_all_, clf_new.feature_all_, rtol=1e-10,
                       err_msg="Feature all counts don't match")
    ...
    def test_prior_unobserved_targets_compatibility(self):
        ...
        for test_input_sets, description in test_cases:
            ...
            assert_array_equal(pred_original, pred_new,
                             err_msg=f"Predictions don't match for {description}")
    ...
    def test_rdkit_scenario_compatibility(self):
        ...
        assert_array_equal(pred_original, pred_new,
                         err_msg="Predictions should match exactly for synthetic sparse data")
    ...
    def test_joint_log_likelihood_compatibility(self):
        ...
        # Check shapes match
        assert jll_original.shape == jll_new.shape, \
            "Joint log likelihood shapes don't match"
        ...
        try:
            jll_orig_large = clf_original._joint_log_likelihood(
                np.array(test_set_with_large_feature, dtype=object)
            )
        ...
        try:
            ...
            jll_new_large = clf_new._joint_log_likelihood(X_large)
    ...
    def test_probability_distribution_consistency(self):
        ...
        assert_allclose(prob_original.sum(axis=1), 1.0, rtol=1e-6,
                       err_msg="Original probabilities don't sum to 1")
        assert_allclose(prob_new.sum(axis=1), 1.0, rtol=1e-6,
                       err_msg="New probabilities don't sum to 1")
        
        # Check that probabilities are in valid range
        assert np.all(prob_original >= 0) and np.all(prob_original <= 1), \
            "Original probabilities out of range"
        assert np.all(prob_new >= 0) and np.all(prob_new <= 1), \
            "New probabilities out of range"
        ...
        for i, (pred_o, pred_n) in enumerate(zip(pred_original, pred_new)):
            assert prob_original[i, pred_o] == np.max(prob_original[i]), \
                f"Original: max prob doesn't match prediction for sample {i}"
            assert prob_new[i, pred_n] == np.max(prob_new[i]), \
                f"New: max prob doesn't match prediction for sample {i}"
    ...
    def test_edge_cases_consistency(self):
        ...
        assert_array_equal(pred_orig, pred_new, 
                          err_msg="Single class predictions don't match")
