...
def rdkit_sparse_to_sklearn(
    ...
    if output_format == 'dense':
        ...
        raise ValueError(f"Unknown output_format: {output_format}. "
                        "Choose from 'auto', 'dense', 'csr', 'csc'.")
...
class RDKitFingerprintConverter:
    ...
    def convert(
        ...
        return rdkit_sparse_to_sklearn(
            fingerprints,
            n_bits=self.n_bits,
    ...
    def _validate_fingerprints(self, fingerprints):
        ...
        for i, fp in enumerate(fingerprints):
            ...
            valid = (
                hasattr(fp, 'GetOnBits') or
                hasattr(fp, 'GetNonzeroElements') or
                isinstance(fp, (set, dict, list, tuple, np.ndarray))
            )
            ...
            if not valid:
                ...
                try:
                    ...
                    raise ValueError(
                        f"Fingerprint at index {i} is not in a supported format. "
                        f"Got type: {type(fp)}"
                    )
